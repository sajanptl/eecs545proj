...
\begin{itemize}
\item The number of words $N$ in document has Poisson distribution with parameter $\zeta$
\item The topic mixture $\theta$ for the document has Dirichlet distribution with parameter $\alpha$ (with a fixed set of $K$ topics)
\item Then for each word $w_i$ in the document:
\begin{enumerate}
\item Choose a topic $z_i$ based on multinomial distribution with parameter $\theta$ from step (2)
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{graphmodel}
...
There are two methods to learn these two things: collapsed Gibbs sampling (Griffiths and Steyvers, 2002) and variational inference (Jordan et al., 1999, Teh et al., 2006). We first discuss Gibbs sampling method here:
...
We implemented LDA using both Gibbs sampling and inference.

\section{Experimental Evaluation}
\subsection{Setup}
Describe recreation of Document Modeling and Yelp experiment from Blei et. al. here.

\subsection{Metrics}
Describe perplexity here.

\subsection{Results}
Show graphs of the perplexity and histogram of topics.

Discuss results in table if also needed.

Also compare gibbs vs inference in each.
...
\small{
Use Bibtex here. We will only have a few references.
