...
\begin{enumerate}
\item $p(z_j | d_i)  = \frac{\text{number of words assigned to }z_j \text{in }d_i}{\text{total number of words in }d_i}$
\item $p( w_k |  z_j) = \frac{\text{number of words assigned to} z_j \text{in } d_i}{\text{number of words assigned to }z_j \text{for all docs}}$
\end{enumerate}
\end{enumerate}
...
However, since the KL divergence is intractable, Jensen's inequality is applied to yield an evidence lower bound. Minimizing the KL divergence is equivalent to maximizing the evidence lower bound [5]. The posterior distribution can be obtained with those variational parameters using EM algorithm. In the maximization step, the gradient $g(\alpha^{t})$ and Hessian $H(\alpha^{t})$ are calculated using the evidence lower bound
...
In the maximization step, the gradient $g(\alpha^{t})$ and Hessian $H(\alpha^{t})$ are calculated using the evidence lower bound $L(\gamma,\phi;\alpha,\beta)$:\\
$g(\alpha^{t})$
