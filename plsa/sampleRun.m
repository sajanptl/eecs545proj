...
Learn.Folding_Iterations = 20; % for TEM only: number of fiolding
                               % in iterations
Learn.saveit = 25;
Learn.sfile = 'plsi_out.mat';
Learn.TEM = 1; %tempered or not tempered
%
%%
% start pLSA
Xtrain = sparse(double(W'));
Xtest = sparse(double(W_test'));

%%
[Pw_z,Pz_d,Pd,Li,perp,beta] = pLSA(Xtrain,[],10,Learn);

% fold in test data
%Pz_q = pLSA_EMfold(Xtest,Pw_z,[],2,beta);


%Learn.TEM = 0;
%[Pw_z2,Pz_d2,Pd2,Li] = pLSA(Xtrain,[],10,Learn);

Pq = sum(Xtest)./sum(Xtest(:));


% fold in test data
%Pz_q2 = pLSA_EMfold(Xtest,Pw_z2,[],2,1);
